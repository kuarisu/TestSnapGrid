...
public class LevelGeneratorWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        m_TallWall = (Material)Resources.Load("m_TallWall");
        m_SmallWall = (Material)Resources.Load("m_SmallWall");
        m_TilePrefab = Resources.Load("Tile") as GameObject;
        ...
        if (GUILayout.Button("Generate Grid"))
        {
            if (m_ColumnsNumber > 0 && m_RowsNumber > 0)
            {
                ...
                for (int x = 0; x < m_ColumnsNumber; ++x)
                {
                    ...
                    {
                        GameObject t = ((GameObject)Instantiate(m_TilePrefab, new Vector3(x, 0, y), new Quaternion()));
        ...
        else
        {
            ...
            for (int i = 0; i < m_Map.Count; i++)
            {
                for (int j = 0; j < m_Map[0].Count; j++)
                {
                    if (m_Map[i][j].GetComponent<Renderer>().sharedMaterial.name.Contains("m_EmptyTile") && (i + j) % 2 == 1)
                    {
                        m_Map[i][j].GetComponent<Renderer>().material = m_EmptyTile;
                    }
    ...
    List<GameObject> CreateColumn(float _x, float _z, int _columnHeight)
    {
        ...
        {
            GameObject t = ((GameObject)Instantiate(m_TilePrefab, new Vector3(_x, 0, _z + z), new Quaternion()));
            column.Add(t);
        }
        return column;
    }
    ...
    List<GameObject> CreateRow(float _x, float _z, int _rowHeight)
    {
        ...
        {
            GameObject t = ((GameObject)Instantiate(m_TilePrefab, new Vector3(_x + x, 0, _z), new Quaternion()));
            _row.Add(t);
        }
        return _row;
    }
